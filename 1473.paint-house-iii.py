...
class Solution:
    def minCost(self, houses: List[int], cost: List[List[int]], m: int, n: int, target: int) -> int:
        @lru_cache(None)
        def dfs(i, tar, last_color):
            if i == m and tar == target:
                return 0
            if i >= m or tar > target:
                return inf

            res = inf
            if houses[i]:
                res = min(
                    res, dfs(i+1, tar + (houses[i] != last_color), houses[i]))
            else:
                for color in range(1, n+1):
                    res = min(res, cost[i][color - 1] + dfs(i+1,
                              tar + (color != last_color), color))
            return res
