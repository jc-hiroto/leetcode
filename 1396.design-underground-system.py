...
class UndergroundSystem:
    ...
    def checkIn(self, id: int, stationName: str, t: int) -> None:
        self.in_record[id] = {"start": stationName, "time": t}

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        checkin = self.in_record[id]
        if checkin["start"] in self.time_record:
            if stationName in self.time_record[checkin["start"]]:
                record = self.time_record[checkin["start"]][stationName]
                record[0] += t-checkin["time"]
                record[1] += 1
            else:
                self.time_record[checkin["start"]][stationName] = [t-checkin["time"], 1]
        else:
            self.time_record[checkin["start"]] = {stationName: [t-checkin["time"], 1]}
        del self.in_record[id]

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        record = self.time_record[startStation][endStation]
